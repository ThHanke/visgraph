...
function ensureSummary() {
  ...
  if (!window.__VG_DEBUG_SUMMARY__) {
    ...
    try {
      window.__VG_DEBUG_SUMMARY__.fallbacks = [];
    } catch (_) {
      try {
...
function recordRecentEvent(fp: string, caller?: string) {
  ...
  while (
    ...
    now - recentEvents[0].ts > DUPLICATE_WINDOW_MS
  ) {
    recentEvents.shift();
  }
  ...
  try {
    ...
    try {
      ...
      if (occ.length >= DUPLICATE_THRESHOLD) {
        try {
          (s.anomalies = s.anomalies || []).push({
            event: eventName,
            fingerprint: fp,
            occurrences: occ.slice(),
            ts: nowIso(),
          });
        } catch (_) {
          try {
    ...
    if (
      options?.captureStack ||
      (typeof window !== "undefined" && window.__VG_DEBUG_STACKS__)
    ) {
      try {
    ...
    try {
      ...
      if (Array.isArray(s.fallbacks) && s.fallbacks.length > 20000)
        s.fallbacks.shift();
    } catch (_) {
      try {
        if (typeof fallback === "function") {
...
try {
  ...
  try {
    if (
      ...
      (import.meta as any).env.VITE_VG_DEBUG === "true"
    ) {
      try {
        window.__VG_DEBUG__ = true;
      } catch (_) {
  ...
  try {
    if (
      typeof import.meta !== "undefined" &&
      (import.meta as any).env &&
      (import.meta as any).env.VITE_VG_DEBUG_STACKS === "true"
    ) {
      try {
        window.__VG_DEBUG_STACKS__ = true;
      } catch (_) {
        try {
          if (typeof fallback === "function") {
            fallback("emptyCatch", { error: String(_) });
...
try {
  if (typeof globalThis !== "undefined") {
    ...
    try {
      (globalThis as any).fallback = fallback;
    } catch (_) {
      try {
        if (typeof fallback === "function") {
          fallback("emptyCatch", { error: String(_) });
