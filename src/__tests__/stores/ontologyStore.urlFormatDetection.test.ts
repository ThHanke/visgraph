...
`;
    ...
    await store.loadKnowledgeGraph(url, { onProgress: () => {}, timeout: 5000 });

    expect(mockFetch).toHaveBeenCalled();
    const calledInit = mockFetch.mock.calls[0][1] || {};
    const acceptHeader = calledInit.headers ? (calledInit.headers['Accept'] || calledInit.headers.Accept) : undefined;
    if (typeof acceptHeader === 'string') {
      expect(acceptHeader).toMatch(/text\/turtle|application\/ld\+json|application\/rdf\+xml/);
    } else {
      const headerObj = calledInit.headers || {};
      const foundAccept = Object.values(headerObj).join(' ');
      expect(foundAccept).toMatch(/text\/turtle|application\/ld\+json|application\/rdf\+xml/);
    }
    ...
    const found = all.some((q: any) => {
      try {
        ...
        return typeof obj === 'string' && (obj.includes('Specimen') || obj.endsWith('/Specimen') || obj.endsWith('#Specimen') || obj.includes(':Specimen'));
      } catch {
        return false;
      }
    });
  ...
  it('loads RDF/XML from a .owl file URL and persists triples (count increases)', async () => {
    ...
    const url = 'https://materialdigital.github.io/logistics-application-ontology/ontology.owl';
  ...
  <rdf:Description rdf:about="http://example.com/subject">
    ...
    const mockFetch = vi.fn(async (input: any, init?: any) => {
      return {
        ...
        headers: {
          get: (name: string) => {
            if (name.toLowerCase() === 'content-type') return 'application/rdf+xml';
            return null;
          }
        },
        text: async () => rdfXml
    ...
    const found = all.some((q: any) => {
      try {
        ...
        return (typeof subj === 'string' && subj.includes('example.com/subject')) ||
               (typeof obj === 'string' && (obj.includes('Specimen') || obj.endsWith('/Specimen') || obj.endsWith('#Specimen')));
      } catch {
        return false;
      }
    });
