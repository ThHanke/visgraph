...
describe('OntologyStore - Graph Preservation', () => {
  ...
  it('should preserve existing graph changes when loading additional ontology', async () => {
    ...
    expect(store.currentGraph.nodes).toHaveLength(2);
    expect(store.currentGraph.edges).toHaveLength(1);
    expect(store.currentGraph.nodes[0].data.literalProperties).toHaveLength(2);
    ...
    // Verify that original nodes are preserved
    const currentNodes = store.currentGraph.nodes;
    const currentEdges = store.currentGraph.edges;
    ...
    // Check that RDF manager was updated with the changes
  ...
  it('should update RDF store when entity properties are modified', async () => {
    ...
    store.updateEntity(entityUri, updates);
    ...
    const preservedNode = store.currentGraph.nodes.find(n => n.id === 'test1');
    expect(preservedNode).toBeDefined();
    expect(preservedNode?.data.literalProperties).toHaveLength(2);
    expect(preservedNode?.data.literalProperties.find(p => p.key === 'foaf:name')?.value).toBe('Test Person');
    expect(preservedNode?.data.literalProperties.find(p => p.key === 'foaf:age')?.value).toBe('25');
  });
  ...
  it('should not duplicate nodes when loading ontology with same entities', async () => {
    ...
    const nodeIds = store.currentGraph.nodes.map(n => n.id);
    const uniqueNodeIds = [...new Set(nodeIds)];
    expect(nodeIds).toHaveLength(uniqueNodeIds.length);

    // Should have the original node plus new ones, but no duplicates
    expect(store.currentGraph.nodes.length).toBeGreaterThan(1);
    expect(store.currentGraph.nodes.find(n => n.id === 'node1')).toBeDefined();
  });
});