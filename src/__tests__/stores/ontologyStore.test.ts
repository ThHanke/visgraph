...
describe('Ontology Store', () => {
  ...
  describe('loadOntology', () => {
    it('should load mock FOAF ontology', async () => {
      ...
      expect(store.loadedOntologies).toHaveLength(1);
      expect(store.loadedOntologies[0].name).toBe('FOAF');
      expect(store.availableClasses.length).toBeGreaterThan(0);
      expect(store.availableProperties.length).toBeGreaterThan(0);
    });
    ...
    it('should accumulate multiple ontologies', async () => {
      ...
      expect(store.loadedOntologies).toHaveLength(2);
      expect(store.availableClasses.some(c => c.namespace === 'foaf')).toBe(true);
      expect(store.availableClasses.some(c => c.namespace === 'org')).toBe(true);
    });
  });
  ...
  describe('setCurrentGraph', () => {
    it('should update current graph', () => {
      ...
      expect(store.currentGraph.nodes).toEqual(nodes);
      expect(store.currentGraph.edges).toEqual(edges);
    });
  });
