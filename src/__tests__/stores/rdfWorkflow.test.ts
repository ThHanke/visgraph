...
describe('RDF Store Workflow Integration Tests', () => {
    ...
    it('should update RDF store when entity properties are modified via NodePropertyEditor', async () => {
      ...
      const updatedNodeData = {
        ...
        annotationProperties: [
          { property: 'rdfs:label', key: 'rdfs:label', value: 'Specimen Length Property', type: 'xsd:string' },
          { property: 'rdfs:comment', key: 'rdfs:comment', value: 'Length measurement of specimen', type: 'xsd:string' }
        ]
      };

      // This should happen in the canvas when NodePropertyEditor calls onSave
      store.updateEntity(entityUri, {
        annotationProperties: updatedNodeData.annotationProperties.map(prop => ({
      ...
      const updatedNodes = currentGraph.nodes.map(node => {
        if (node.data.uri === entityUri) {
          return {
            ...
            data: {
              ...node.data,
              literalProperties: updatedNodeData.annotationProperties.map(prop => ({
                key: prop.key,
                value: prop.value,
                type: prop.type
              }))
      ...
      expect(commentQuads[0].object.value).toBe('Length measurement of specimen');
    });
  ...
  describe('Export Functionality', () => {
    it('should export complete RDF store including user modifications', async () => {
      ...
      store.updateEntity('http://example.com/testInstance', {
        type: 'ex:TestClass',
        annotationProperties: [
          { propertyUri: 'rdfs:label', value: 'My Test Instance', type: 'xsd:string' },
          { propertyUri: 'rdfs:comment', value: 'This is a test', type: 'xsd:string' },
          { propertyUri: 'ex:customProperty', value: 'custom value', type: 'xsd:string' }
        ]
      });
  ...
  describe('Reasoner Integration', () => {
    it('should use RDF store data for reasoning', async () => {
      ...
      store.updateEntity('http://example.com/john', {
        type: 'foaf:Person',
        annotationProperties: [
      ...
      store.updateEntity('http://example.com/acme', {
  ...
  describe('RDF Manager Direct Tests', () => {
    it('should properly expand prefixes', () => {
      ...
      expect(rdfManager.expandPrefix('rdfs:label')).toBe('http://www.w3.org/2000/01/rdf-schema#label');
      expect(rdfManager.expandPrefix('foaf:name')).toBe('http://xmlns.com/foaf/0.1/name');
      expect(rdfManager.expandPrefix('http://example.com/full')).toBe('http://example.com/full');
    });
    ...
    it('should update and remove entity properties correctly', async () => {
      ...
      rdfManager.updateEntity(entityUri, {
        annotationProperties: [
          { propertyUri: 'rdfs:label', value: 'Initial Label', type: 'xsd:string' },
          { propertyUri: 'rdfs:comment', value: 'Initial Comment', type: 'xsd:string' }
        ]
      });
      ...
      let labelQuads = store.getQuads(entityUri, 'http://www.w3.org/2000/01/rdf-schema#label', null, null);
      expect(labelQuads).toHaveLength(1);
      ...
      rdfManager.updateEntity(entityUri, {
        annotationProperties: [
          ...
          { propertyUri: 'ex:newProperty', value: 'New Value', type: 'xsd:string' }
        ]
      });

      // Verify old label is replaced
      labelQuads = store.getQuads(entityUri, 'http://www.w3.org/2000/01/rdf-schema#label', null, null);
      expect(labelQuads).toHaveLength(1);
      expect(labelQuads[0].object.value).toBe('Updated Label');

      // Verify comment is removed (not in update)
      const commentQuads = store.getQuads(entityUri, 'http://www.w3.org/2000/01/rdf-schema#comment', null, null);
      expect(commentQuads).toHaveLength(0);

      // Verify new property is added
      const newPropQuads = store.getQuads(entityUri, 'http://example.com/newProperty', null, null);
    ...
    it('should export with correct prefixes and formatting', async () => {
      ...
      // Add some test data
      rdfManager.updateEntity('http://example.com/person1', {
        type: 'foaf:Person',
        annotationProperties: [
