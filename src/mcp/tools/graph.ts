...
const help: McpTool = {
  ...
  async handler(params): Promise<McpResult> {
    ...
    const instructions = [
      ...
      '',
      'WRONG (silently ignored): {"tool":"addNode","input":{"iri":"ex:Pizza"}}',
      'WRONG (silently ignored): any native tool/function-call syntax your model normally uses',
      'WRONG (silently ignored): {"jsonrpc":"2.0","method":"addNode","params":{"iri":"ex:Pizza"},"id":1}  ← method must be "tools/call", NOT the tool name',
      'RIGHT: `{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"addNode","arguments":{"iri":"ex:Pizza","typeIri":"owl:Class"}}}`',
      '',
