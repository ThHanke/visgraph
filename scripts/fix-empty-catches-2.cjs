/**
 * scripts/fix-empty-catches-2.cjs
 *
 * Finds catch blocks whose bodies only contain comments or whitespace, e.g.:
 *   catch (e) { /* ignore *\/ }
 *   catch (err) { // ignore }
 *
 ...
 *
 * Usage: node scripts/fix-empty-catches-2.cjs
 */
...
function fixFile(file) {
  ...
  // Match catch blocks where the body is only whitespace and/or comments (single-line // or block /* */).
  // Captures the catch binding (e.g., e or err) in group 1.
  const re = /catch\s*\(\s*([^\)]+?)\s*\)\s*\{\s*(?:(?:\/\*[\s\S]*?\*\/)|(?:\/\/[^\n]*\n?))+\s*\}/gs;

  let replaced = 0;
  const newSrc = src.replace(re, (match, p1) => {
    replaced++;
    const binding = (p1 || 'e').trim();
