...
/**
 * scripts/playwright-css-usage.js
 *
 * Starts the dev server (npm run dev), detects the local URL from stdout,
 * launches a Playwright Chromium instance, navigates to the app, listens for
 * console messages and stops when no new console message appears for 2s.
 *
 * Outputs a JSON report to ./playwright-reports/css-usage-report.json
 *
 * Note: this script requires `playwright` to be installed in the project.
 * Install with: `npm i -D playwright` (or `npm i -D @playwright/test` and adjust imports).
 *
 * Usage:
 *   node scripts/playwright-css-usage.js
 */
...
async function waitForUrlFromStdout(child, timeout = 30000) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', (chunk) => {
      ...
      stdoutBuffer += text;
      // try to find url in the most recent chunk or in buffered stdout
    ...
    child.stderr.on('data', (chunk) => {
      const text = String(chunk);
      // sometimes vite emits the server url on stderr, try parsing there too
...
async function run() {
  ...
  let pw;
  try {
    pw = require('playwright'); // expects playwright to be installed
  } catch (err) {
  ...
  page.on('console', (msg) => {
    try {
      // Print to main process stdout with type prefix to help debugging
  ...
  // Keep track of console messages in an array for post-processing (simple)
  const consoleEntries = [];
  page.on('console', (msg) => {
    consoleEntries.push({ type: msg.type(), text: msg.text(), ts: Date.now() });
  });

  // Wait until there's a 2s period with no new console messages.
  const QUIET_MS = 2000;
  const MAX_RUN_MS = 120000; // safety cap of 2 minutes
  ...
  // Evaluate page to collect classes, computed styles, and style sheet hrefs
  const report = await page.evaluate(async () => {
    ...
    // For computed styles we will sample one element per class (first found)
    ...
    // Attempt to fetch stylesheet contents (may be cross-origin for third-party; skip those)
    const cssFiles = {};
